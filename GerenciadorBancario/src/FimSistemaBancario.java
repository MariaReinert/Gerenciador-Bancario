...
public class FimSistemaBancario {
}
public static void main(String[] args) {
    ...
    while (true) {
        ...
        if (opcao == 1) {
            ...
            int iOrigem = contas.indexOf(origem);
            int iDestino = contas.indexOf(destino);

            if (iOrigem != -1 && iDestino != -1 && saldos.get(iOrigem) >= valor) {
                saldos.set(iOrigem, saldos.get(iOrigem) - valor);
                saldos.set(iDestino, saldos.get(iDestino) + valor);
                historicos.set(iOrigem, historicos.get(iOrigem) + "Transferência de R$ " + valor + " para " + destino + "\n");
                historicos.set(iDestino, historicos.get(iDestino) + "Recebido R$ " + valor + " de " + origem + "\n");
